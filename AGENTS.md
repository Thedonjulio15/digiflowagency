...
This document provides an overview of the DigiFlow Agency website project structure.

## Project Overview

A high-converting marketing website for DigiFlow Agency. Built with TanStack Start (React 19 SSR) and deployed on Netlify. The site is designed to qualify and capture leads via a multi-step Netlify Form, while proving agency capability through portfolio case studies and testimonials.

## Key Files

- `src/routes/index.tsx` — Entire single-page site (Nav, Hero, Services, Process, Portfolio, Testimonials, About, LeadForm, FAQ, Footer)
- `src/routes/__root.tsx` — HTML shell with Google Fonts and global meta
- `src/styles.css` — Tailwind + custom CSS (brand tokens, animations)
- `public/digiflow-logo.jpeg` — Brand logo
- `public/__forms.html` — Netlify Forms build-time detection skeleton **DO NOT DELETE**

## Netlify Forms

The lead qualification form submits to `/__forms.html` (not `/`). If field names change, update **both** `public/__forms.html` and the `LeadForm` component in `index.tsx`.

## Brand

- Primary: `#1A1AFF` (electric blue)
- Background: `#08081A` (deep navy)
- Text: white with opacity variants (`/60`, `/40`, etc.)
